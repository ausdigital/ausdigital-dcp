...
A draft [REST interface v1.0](http://ausdigital.org/specs/ausdigital-dcp/1.0/api) is available for comment.
...
The Digital Capability Publisher is functionally equivalent to a REST version of the European OASIS standard "Service Metadata Publisher". Users are expected to have used the `ausdigital-dcl/1` to discover the appropriate DCP service for a particular business. They then interact with the DCP using the REST protocol [documented here](http://ausdigital.org/specs/ausdigital-dcp/1.0/api) to access a collection of ServiceMetadata entries for the business.
